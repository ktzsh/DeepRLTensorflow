    ...
    class Worker(StoppableThread, ShareSessionThread):
        ...
        def run(self):
            with self.default_sess():
                player = get_player_fn(ROM_FILE, train=False)
...
def eval_model_multithread(pred, nr_eval, get_player_fn, ROM_FILE):
    """
Args:
...
"""
    ...
    with pred.sess.as_default():
        mean, max = eval_with_funcs(
            [pred] * NR_PROC, nr_eval, get_player_fn, ROM_FILE, verbose=True)
...
class Evaluator(Callback):
    ...
    def _trigger(self):
        ...
        mean, max = eval_with_funcs(self.pred_funcs, self.eval_episode,
                                    self.get_player_fn, self.ROM_FILE)
        t = time.time() - t
