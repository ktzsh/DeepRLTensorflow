...
from tensorpack.tfutils import (varreplace, summary, get_current_tower_context,
                                optimizer, gradproc)
...
class Model(ModelDesc):
	...
	def inputs(self):
		...
		return [
		    tf.placeholder(
		        tf.uint8,
		        (None, ) + self._shape2d + ((self.history + 1) * self.channel, ),
		        'comb_state'),
	...
	def build_graph(self, comb_state, action, reward, isOver):
		...
		comb_state = tf.reshape(
		    comb_state,
		    [-1] + list(self._shape2d) + [self.history + 1, self.channel])
		...
		next_state = tf.slice(
		    comb_state, [0, 0, 0, 1, 0], [-1, -1, -1, self.history, -1],
		    name='next_state')
		...
		pred_action_value = tf.reduce_sum(self.predict_value * action_onehot,
		                                  1)  # N,
		max_pred_reward = tf.reduce_mean(
		    tf.reduce_max(self.predict_value, 1), name='predict_reward')
	...
	def optimizer(self):
		lr = tf.get_variable(
		    'learning_rate', initializer=self.learning_rate, trainable=False)
	...
	def update_target_param():
		...
		for v in vars:
			...
			if target_name.startswith('target'):
				...
				logger.info("Target Network Update: {} <- {}".format(
